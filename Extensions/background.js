...
async function CheckParts() {
  ...
  let storage = await Storage(),
    store = { bank: true, site: true, VPN: false, Translate: false, lablevpn: {}, theme: true },
    nextRl = new Date().getTime() + 45 * 60000;

  // Promise.all([api, Storage()]).then(([contorols, local]) => {
  URI('./contorols-v2.json').then(contorols => {
    ...
    Storage("set", { ...store, ...storage, contorols: { ...storage?.contorols, ...contorols, timeAd: 0, timeRl: nextRl, keys: "" } });
    ...
    Storage('set', { ...store, ...storage, contorols: { ...storage?.contorols, AD: [], timeRl: 0, timeAd: 0 } });
    // Storage('set', updStage);
  });
...
async function Handel_code(tabId, stage) {
  ...
  if (data.active && (check.protocol == "https:" || check.protocol == "http:")
