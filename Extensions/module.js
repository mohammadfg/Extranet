const { action, tabs, runtime, storage } = chrome;

function Icon(path = "./icon/128.png", text = "", RGB = "#fff") {
  try {
    action.setIcon({ path: path });
    action.setBadgeText({ text: text });
    action.setBadgeBackgroundColor({ color: RGB });
  } catch (error) {
    Icon();
  }
